...
BookDatabase::BookDatabase( const std::string & filename )
{
  ...
  ///////////////////////// TO-DO (2) //////////////////////////////
    ...
    while (fin >> book)
    {
      _books_vector.push_back (std::move(book)); 
    }
...
///////////////////////// TO-DO (3) //////////////////////////////
  ...
  {
    return find (_books_vector.begin(), isbn);
  }

  Book * BookDatabase::find (std::vector<Book>::iterator currentBook, const std::string & isbn )
  {
    //base case
    if (_books_vector.end() == currentBook)
    return nullptr; 
    ...
    //recurse
    return find ( std::next(currentBook), isbn);
