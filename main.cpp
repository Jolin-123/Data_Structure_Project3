  ...
  void carefully_move_books( std::size_t quantity, std::stack<Book> & broken_cart, std::stack<Book> & working_cart, std::stack<Book> & spare_cart )
  {
    ///////////////////////// TO-DO (1) //////////////////////////////
      ...
      if ( quantity == 1 )
      {
       ...
       //just before kicking off the recursion and then just after each move
      }
  ...
  void carefully_move_books( std::stack<Book> & from, std::stack<Book> & to )
  {
    ///////////////////////// TO-DO (2) //////////////////////////////
      ...
      if (!from.empty())
      {
        std::stack<Book> temp;
        trace (from, to, temp);
        carefully_move_books (from.size(), from, to, temp);
      }
...
int main( int argc, char * argv[] )
{
  ...
  ///////////////////////// TO-DO (4) //////////////////////////////
    ...
    myCart.push( { "Hunger Games"    , ""      , "9780545310581" } );
    myCart.push( { "Eat pray love"   , "Asher" , "9780399576775" } );
    myCart.push( { "Les Mis"         , ""      , "0140444300"    } );
    myCart.push( { "131 Answer Key"  , ""      , "54782169785"   } );
    myCart.push( { "Like the Animals", ""      , "9780895656926" } );
  ...
  ///////////////////////// TO-DO (7) //////////////////////////////
    ...
    {
      checkoutCounter.pop();
      auto ptr = storeDataBase.find( checkoutCounter.front().isbn() );
      if( ptr != nullptr )
      {
        amountDue += ptr->price();
        std::cout << *ptr << '\n';
      ...
      {
        std::cout << " \" " << checkoutCounter.front().isbn() << " \" ( " << checkoutCounter.front().title() << " ) , the book wasn't found and there will be no charge. \n";
      }
    }
